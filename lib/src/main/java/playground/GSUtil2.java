...
public class GSUtil2 {
	...
	public static void calculateGraphSegments(List<GraphSegment> graphArray, double fiveMinVol,
		    ...
		    Map<String, Double> priceWithTime) {
		//time = timeNMinsAgo(time, 5);
		priceWithTime.put(time, fiveMinClose);
		String time5MinsAgo = Util.timeNMinsAgo(time, 5);
		double fiveMinPrevClose = (priceWithTime.containsKey(time5MinsAgo)) ? priceWithTime.get(time5MinsAgo) : fiveMinOpen;
    	if (fiveMinClose >= fiveMinPrevClose) {
          if (graphArray.size() == 0) {
              graphArray.add(new GraphSegment("c", "06:30", time, fiveMinPrevClose, fiveMinClose, fiveMinClose, 0));
          ...
          if (prevGS.identifier.equals("c")) {
              ...
              if (diffTime(prevGS.endTime, time) >= 30) {
            	  ...
            	  if (((fiveMinClose - prevGS.endPrice) / prevGS.endPrice) * 100 > (ninetyPercentileBarChange / 10)) {
          ...
          /*else if (prevGS.identifier.equals("ur")) {
              ...
              } else if (diffTime(prevGS.pullbackTime.peekLast(), time) >= 20) {
        	  ...
        	  if (((fiveMinClose - prevGS.endPrice) / prevGS.endPrice) * 100 
            		  > Math.max(ninetyPercentileBarChange, Math.abs(((prevGS.endPrice - prevGS.startPrice) / prevGS.startPrice) * 100) * 0.5)) {
                  //prevGS.identifier = "d";
                  int barcount4NewSegment = diffTime(prevGS.endTime, time) / 5;
            	  prevGS.barCount = prevGS.barCount - barcount4NewSegment + 1;
                  graphArray.add(new GraphSegment("u", prevGS.endTime, time, prevGS.endPrice, fiveMinClose, fiveMinClose, barcount4NewSegment));
        	  ...
        	  /*else if (((fiveMinClose - prevGS.endPrice) / prevGS.endPrice) * 100 > Math.abs(((prevGS.endPrice - prevGS.startPrice) / prevGS.startPrice) * 100) * 0.33) {
                  prevGS.currentPrice = fiveMinClose;
          ...
          /*else if (prevGS.identifier.equals("dr")) {
              if (((fiveMinClose - prevGS.endPrice) / prevGS.endPrice) * 100 
            	  ...
            	  if (((fiveMinClose - prevGS.pullbackPrice.peekLast()) / prevGS.pullbackPrice.peekLast()) * 100 > (ninetyPercentileBarChange / 10)) {
    	...
    	} else if (fiveMinClose < fiveMinPrevClose) {
          if (graphArray.size() == 0) {
        	  graphArray.add(new GraphSegment("c", "06:30", time, fiveMinPrevClose, fiveMinClose, fiveMinClose, 0));
          ...
          if (prevGS.identifier.equals("c")) {
        	  ...
        	  if (diffTime(prevGS.endTime, time) >= 30) { // Math.abs(((fiveMinClose - prevGS.endPrice) / prevGS.endPrice) * 100) < ninetyPercentileBarChange &&
            	  ...
            	  if (((fiveMinClose - prevGS.endPrice) / prevGS.endPrice) * 100 < (-1 * ninetyPercentileBarChange / 10)) {
                  //if (fiveMinClose < prevGS.endPrice) {
                      prevGS.endTime = time;
                      prevGS.endPrice = fiveMinClose;
                  }
          ...
          /*else if (prevGS.identifier.equals("dr")) {
        	  ...
        	  if (((prevGS.endPrice - fiveMinClose) / prevGS.endPrice) * 100 
            		  > Math.max(ninetyPercentileBarChange, Math.abs(((prevGS.endPrice - prevGS.startPrice) / prevGS.startPrice) * 100) * 0.5)) {
                  //prevGS.identifier = "u";
                  int barcount4NewSegment = diffTime(prevGS.endTime, time) / 5;
            	  prevGS.barCount = prevGS.barCount - barcount4NewSegment + 1;
        	  ...
        	  /*else if (((prevGS.endPrice - fiveMinClose) / prevGS.endPrice) * 100 > Math.abs(((prevGS.endPrice - prevGS.startPrice) / prevGS.startPrice) * 100) * 0.33) {
                  prevGS.currentPrice = fiveMinClose;
                  prevGS.pullbackPrice.add(fiveMinClose);
          ...
          /*else if (prevGS.identifier.equals("ur")) {
              if (((prevGS.endPrice - fiveMinClose) / prevGS.endPrice) * 100 
            	  ...
            	  if (((fiveMinClose - prevGS.pullbackPrice.peekLast()) / prevGS.pullbackPrice.peekLast()) * 100 < (-1 * ninetyPercentileBarChange / 10)) {
                  //if (fiveMinClose < prevGS.pullbackPrice.peekLast()) {
                      prevGS.pullbackTime.removeLast(); prevGS.pullbackTime.add(time);
                      prevGS.pullbackPrice.removeLast(); prevGS.pullbackPrice.add(fiveMinClose);
                  }
